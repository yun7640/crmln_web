...
작성일: 2026-07-26 · 최종 갱신: 2026-07-27 · 최신 버전: **v17** (T0–T5 완료)
...
- **T6. 과거 회차(2023.1–2025.1) 측정 원본 확보 후 소급 누적** — T5로 업로드 경로는 열렸으나
  2025.7·2026.1 외 과거 회차의 **측정 원본 엑셀이 아직 리포/세션에 없다**. 원본을 확보하면
  ⑥탭 소급 누적(`reference=1`)으로 올려 ④탭 'DCM QC bias vs 평가 bias' 그래프의 짝 데이터를 늘릴 수 있다.
  · 현재 짝 데이터 2개(2025.7·2026.1) — **3개 미만이라 상관 r은 계속 보류**(v15 방침 유지).
  · 소급 저장분은 `reference=True`로 표시되고 `stats.drift`에서 자동 제외된다.

- **T7. `DCM_SEL_SHEET` 이름 고정 문제** — 출력 선택 시트명이 `'2026.7_결과선택'`으로 상수 고정이라
  2026.1 측정지를 올려도 시트명·제목이 '2026-07 회차'로 나온다. 과거 회차 검토파일에서는 오해 소지가 있다.
  · 회차 라벨을 받아 동적으로 붙이는 편이 옳으나, 스모크·⑥탭이 이 상수명을 참조하므로 **함께 고쳐야 한다**.
  · 우선순위 낮음(표시 문제이며 계산값에는 영향 없음).

### 진행 중/기타
- 중복 회차 라벨(2026-07 / 2026.07 / 2026-07-01) 정리는 사용자가 ⑥ 탭 삭제표에서 수행, `2026.7`만 유지.
- **T0–T5 전부 완료.** 남은 사용자 조치: ① GitHub Desktop에서 Push, ② Drive `crmln-app` 폴더 삭제(T3),
